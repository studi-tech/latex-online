...
LatexOnline.create("/tmp/downloads/", "/tmp/storage/").then(onInitialized);
...
async function handleResult(res, preparation, force, downloadName) {
  ...
  if (force && compilation) latexOnline.removeCompilation(compilation);
...
app.get("/compile", async (req, res) => {
  ...
  var preparation;
  if (req.query.text) {
    preparation = await latexOnline.prepareTextCompilation(
      req.query.text,
      command
    );
  } else if (req.query.url) {
    preparation = await latexOnline.prepareURLCompilation(
      req.query.url,
      command
    );
  ...
  if (preparation)
    ...
    sendError(
      res,
      "ERROR: failed to parse request: " + JSON.stringify(req.query)
    );
});

app.post("/studi", async (req, res) => {
  var forceCompilation = req.query && !!req.query.force;
  var command = req.query && req.query.command ? req.query.command : "pdflatex";
  command = command.trim().toLowerCase();
  ...
  var preparation = await latexOnline.prepareStudiCompilation(
    body.latex,
    body.imgs,
    command
  );
...
app.post("/data", upload.any(), async (req, res) => {
  ...
  var preparation = await latexOnline.prepareTarballCompilation(
    file.path,
    req.query.target,
    command
  );
