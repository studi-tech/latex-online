...
class LatexOnline extends EventEmitter {
  ...
  static async create(tmpFolder, resultsFolder) {
    ...
    if (!downloadManager || !resultsFolderSuccess) return null;
  ...
  async prepareGitCompilation(url, target, branch, command, workdir) {
    var { request, userError } = await CompilationRequest.createGitRequest(
      url,
      target,
      branch,
      command,
    ...
    if (!request) return new LatexOnline.Result(null, null, userError);
  ...
  async prepareURLCompilation(url, command) {
    ...
    if (!folderPath) return new LatexOnline.Result(null, null, userError);
    var { request, userError } = await CompilationRequest.createFileRequest(
   ...
   * @return {!Promise<!LatexOnline.Result>}
   */
  async prepareStudiCompilation(text, imgs, command) {
    var fileName = "main.tex";
    var downloader = this._downloadManager.createStudiDownloader(
      text,
      imgs,
      fileName
    );
  ...
  async prepareTextCompilation(text, command) {
    var { request, userError } = await CompilationRequest.createForText(
      text,
      command
    );
    if (!request) return new LatexOnline.Result(null, null, userError);
    var downloader = this._downloadManager.createTextDownloader(
      text,
      request.target
    );
    return new LatexOnline.Result(request, downloader, null);
