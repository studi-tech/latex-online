...
var UserErrors = {
  BranchNotFound: (url, branch) =>
    `failed to find branch '${branch}' at ${url}`,
  BranchAmbiguity: (url, branch) =>
    `multiple branches at ${url} matched requested ${branch}`,
};
...
class CompilationRequest {
  ...
  static createForText(text, command) {
    ...
    if (userError) return { request: null, userError };
