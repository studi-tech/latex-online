...
var UserErrors = {
  LoadingBadResponse: (url, statusCode) =>
    `failed to download URL ${url} - got response status ${statusCode}`,
...
class DownloadManager {
  ...
  static async create(folderPath) {
    ...
    if (!success) {
      logger.error(
        "ERROR: failed to create folder for Downloader - " + folderPath
      );
      return null;
    }
  ...
  createTextDownloader(text, fileName) {
    ...
    return new Downloader(async () => {
      ...
      if (!success) return new DownloadResult(null, null);
      var filePath = path.join(folderPath, fileName);
      var fileCreated = await utils.writeFile(filePath, text);
      if (!fileCreated) {
        logger.error(
          `ERROR: Downloader failed to write ${text.length} bytes of text into file ${filePath}`
        );
  ...
  createGitDownloader(url) {
    ...
    return new Downloader(async () => {
      ...
      if (exists) {
        logger.error(
          `ERROR: Downloader hit a conflict - directory ${folderPath} already exists`
        );
        return new DownloadResult(null, null);
      }
      var fulfill;
      ...
      exec(
        "git",
        ["clone", "--depth", "1", url, folderPath],
        { timeout: utils.minutes(2) },
        (err, stdout, stderr) => {
          if (err) {
            logger.error(
              `ERROR: Downloader failed to clone git repository. ${err.message}`
            );
   ...
   * @return {!Downloader}
   */
  createStudiDownloader(text, imgs, fileName) {
    var folderPath = this._nextName();
    return new Downloader(async () => {
      var success = await createFolder(folderPath);
      if (!success) return new DownloadResult(null, null);
      ...
      const toDownload = new Set(imgs);
      var imagePromises = Array.from(toDownload).map((img) => {
        var imgFileName = path.basename(img);
        var imgFilePath = path.join(folderPath, imgFileName);
        return utils.downloadFile(img, imgFilePath);
      });

      var results = await Promise.all(imagePromises);
      var failed = results.some((result) => !result);
      if (failed) {
        logger.error(
          `ERROR: Downloader failed to download some images into folder ${folderPath}`
        );
        utils.rmdir(folderPath);
        return new DownloadResult(null, null);
      }
      ...
      request
        ...
        .on("response", function (response) {
          if (response.statusCode < 200 || response.statusCode >= 400) {
            ...
            logger.info(
              `INFO: Downloader tried to load ${url} and got response status ${response.statusCode}`
            );
        ...
        .on("error", function (err) {
          ...
          logger.info(
            `INFO: Downloader tried to load ${url} and got error - ${err}`
          );
          fulfill(new DownloadResult(null, error));
        })
        .pipe(fileHandle);
  ...
  createTarballExtractor(tarballPath) {
    ...
    return new Downloader(async () => {
      ...
      exec(
        "tar",
        ["-xf", tarballPath, "-C", folderPath],
        (err, stdout, stderr) => {
          if (err) {
            utils.rmdirRecursive(folderPath);
            logger.error(
              `ERROR: Downloader failed to extract tarball. ${err.message}`
            );
            fulfill(
              new DownloadResult(null, UserErrors.TarballExtractionFailed())
            );
            return;
          }
          fulfill(new DownloadResult(folderPath, null));
...
class Downloader {
  ...
  dispose() {
    ...
    utils.rmdirRecursive(this.folderPath).then((success) => {
      ...
      else logger.info("Downloader removed folder " + this.folderPath);
    });
  }
}
...
async function createFolder(folderPath) {
  ...
  if (!success)
    logger.error(
      `ERROR: Downloader failed to create temporary directory ${folderPath}`
    );
  return success;
}
