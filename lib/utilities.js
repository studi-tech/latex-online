...
var utils = {
  ...
  mkdir: function (path) {
    ...
    fs.mkdir(
      path,
      createErrorHandler(`ERROR: failed to create directory ${path}.`, fulfill)
    );
    return promise;
  },
  ...
  unlink: function (path) {
    ...
    fs.unlink(
      path,
      createErrorHandler(`ERROR: failed to remove file at ${path}.`, fulfill)
    );
    return promise;
  },
  ...
  rmdir: function (path) {
    ...
    fs.rmdir(
      path,
      createErrorHandler(
        `ERROR: failed to remove directory at ${path}.`,
        fulfill
      )
    );
    return promise;
  ...
  rmdirRecursive: function (path) {
    ...
    var child = exec(
      ...
      ["-rf", path],
      createErrorHandler(
        `ERROR: failed to remove directory recursively at ${path}.`,
        fulfill
      )
    );
    return promise;
  ...
  writeFile: function (path, text) {
    ...
    fs.writeFile(
      ...
      text,
      "utf8",
      createErrorHandler(`ERROR: failed to write file ${path}.`, fulfill)
    );
    return promise;
  },
    ...
    if (url.startsWith("http")) {
      ...
      request
        ...
        .on("error", function (err) {
          logger.error(`ERROR: failed to download file from ${url} - ${err}`);
          fulfill(false);
        })
        .pipe(file);
      ...
      if (!matches || matches.length !== 3) {
        logger.error(`ERROR: failed to parse data URL ${url}`);
        fulfill(false);
      } else {
        ...
        fs.writeFile(destinationPath, data, (err) => {
          if (err) {
            logger.error(
              `ERROR: failed to write data URL content to file ${destinationPath} - ${err}`
            );
            fulfill(false);
          } else {
            fulfill(true);
      ...
      logger.error(`ERROR: cannot download from unsupported URL ${url}`);
      fulfill(false);
    }
    return promise;
  },
